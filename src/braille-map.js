...
// スラッシュ「/」(3,4→「や」と衝突)。
//
// Issue #5でIssue #2の指示訂正が来たが、以下の新たな衝突が判明したため
// 保留中(詳細はPR説明・チャットを参照、要ゆーちゃさん確認):
//   - 読点「、」を5,6に変更する指示 → 外字符(ALPHA_PREFIX)と衝突するため、
//     ひとまず元のIssue #2の指定(4,5)のまま据え置き。
//   - 句点「。」2,5,6 → 既に実装済みのピリオド「.」(2,5,6)と衝突するため未実装。
//
// 疑問符「？」: Issue #5の訂正により点2・6に変更(旧: 点1・5・6、Issue #2の指示)。
// 感嘆符「！」: 点2・3・5(Issue #5で新規追加、衝突なし)。
// 読点「、」: 点4・5(Issue #2の指定を暫定的に維持)。
// ピリオド「.」: 点2・5・6(Issue #2で実装済み)。
